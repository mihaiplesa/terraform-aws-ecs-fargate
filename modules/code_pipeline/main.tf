...
data "template_file" "testspec" {
  template = "${file("${path.module}/buildspecs/test.yml")}"
...
resource "aws_codebuild_project" "app_test" {
  name          = "app-codebuild-test"
  build_timeout = "10"
...
resource "aws_codepipeline" "pipeline" {
  ...
  stage {
    ...
    action {
      ...
      configuration {
        ProjectName = "app-codebuild-test"
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      configuration {
        ProjectName = "app-codebuild-build"
      }
    }
  }
